...
$outFile = Join-Path $desktop 'EMA_Crossover_Template.xlsx'
$tmpFile = Join-Path $desktop 'EMA_Crossover_Template__tmp.xlsx'
...
try {
  ...
  # Add default chart in free area on the far right (W6:AH26)
  $left = $ws.Range('W6').Left
  $top = $ws.Range('W6').Top
  $right = $ws.Range('AH6').Left + $ws.Range('AH6').Width
  $bottom = $ws.Range('W26').Top + $ws.Range('W26').Height
  $chartObj = $ws.ChartObjects().Add($left, $top, ($right-$left), ($bottom-$top))
  ...
  $wb.SaveAs($tmpFile, 51)
  $wb.Close($true)

  if (Test-Path $outFile) { Remove-Item $outFile -Force }
  Move-Item $tmpFile $outFile -Force

  Write-Output "Rewritten with chart: $outFile"
}
finally {
  $excel.Quit()
