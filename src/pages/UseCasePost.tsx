...
import { getUseCaseBySlug, getAllUseCases, getUseCaseStylesUrl, type UseCasePost } from "@/lib/usecase";

const UseCasePostPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const [post, setPost] = useState<UseCasePost | null>(null);
  const [relatedPosts, setRelatedPosts] = useState<UseCasePost[]>([]);
  ...
  useEffect(() => {
    ...
    const styleId = `usecase-styles-${slug}`;
    if (document.getElementById(styleId)) return;

    const link = document.createElement('link');
    link.id = styleId;
    link.rel = 'stylesheet';
    link.href = getUseCaseStylesUrl(slug);
...
export default UseCasePostPage;
